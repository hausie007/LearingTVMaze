...
**Cards**: 4 mission cards + 1 conditional green Join card at index 2.
...
When triggered from the join card, shows a scrollable list of discovered hosts. Each host card shows: host name, IP, player count, theme, mission, and character preview. Pressing a host card → navigates to `join_flow.tscn`.
...
#### Responsive Sizing
Same algorithm as wizard: card sizing, selector sizing, and spacers all derive from `_available_setup_width()` and viewport height.
...
6. **Join Banner** — "How to Join" instructions panel
...
#### Join Banner
4-step instructions with WiFi name auto-detection. Step 3 uses `RichTextLabel` with BBCode to highlight "Play Together" in green bold.
...
Activated when the game starts on the host. The joiner's screen becomes a pure remote controller.

**Layout**:
- Character icon (positioned opposite to D-pad side)
- Title: "Controller"
- Goal label (updated via `rpc_update_remote_goal`)
- Chaser countdown labels (for delayed chaser release)

**All D-pad input forwarded to host**. Back (`ui_cancel`) leaves session and returns to home.
...
#### Goal Updates
Host sends role-specific goal text to each joiner via `rpc_update_remote_goal`. Goals update dynamically (e.g., "Collect all letters" → "Find the exit" after completion).
