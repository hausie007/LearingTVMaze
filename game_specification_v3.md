...
* **Host Lobby (`host_lobby.gd`):** The waiting room before a local multiplayer game begins.
  * **What the User Sees:** A row of player slots `[Slot 1] ... [Slot 4]`. The host occupies Slot 1 (marked with a yellow "You" label). Empty slots display dashed borders that **pulse slowly in opacity (from 45% to 100% using a 1.2s sine wave)**, signaling that the slot is waiting for a player. Below, a scrollable panel provides a 4-step "How to Join" guide that auto-detects the host's current local WiFi network name.
